...
function renderEbayDescription(input, { businessName, logoUrl } = {}) {
  ...
  const title = esc(input.title.trim() || `${businessName?.trim() || "Store"} Product`);
  const make = esc(str(vehicle?.make).trim() || "—");
  const model = esc(str(vehicle?.model).trim() || "—");
  const series = esc(str(vehicle?.model_code).trim() || "—");
  ...
  const mpn = esc(input.mpn.trim() || "—");
  const stockNumber = esc(input.stockNumber.trim() || "—");

  const spnArr = input.supersededPartNumbers.filter((s) => s.trim());
  const superseded = esc(spnArr.length > 0 ? spnArr.join(", ") : "—");

  const authenticity = esc(input.authenticity.trim() || "—");
  const warranty = esc(input.warranty.trim() || "—");
  const condition = esc(CONDITION_LABEL[input.condition] ?? input.condition);
  const notes = esc(input.conditionNotes.trim() || "—");

  const validFitment = input.fitment.filter((r) => str(r.make).trim() || str(r.model).trim());

  const fitmentRows = validFitment.length > 0
    ? validFitment.map((r, i) => {
        const rowBg = i % 2 === 1 ? "background:#0e0e0e;" : "";
        const mk = esc(str(r.make).trim());
        const md = esc(str(r.model).trim());
        const cd = esc(str(r.model_code).trim() || "—");
        const from = str(r.year_from).trim();
        const to = str(r.year_to).trim();
...
module.exports = { renderEbayDescription, descriptionInputFromResolved, isGeneratedEbayDescription };
